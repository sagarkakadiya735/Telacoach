        ...
        <div class="container pb-5">
            <div class="container box-shadow border-rounde mt-5 p-0 bg-white" style="width: 750px">
                <div class="">
                    ...
                    <div class="text-center h5 text-center font-weight-bold border-bottom pr-100 mb-0 pb-4">Telacoach Certification</div>
                        <div class="px-5 pb-5 pt-5 about-text" v-html="certification_data">
                          {{ certification_data }}
                        </div>
                   </div>
            </div>
...
export default {
  ...
  methods: {
    async getTeamsData() {
      var param = "lang=" + "0";
      let res = await apiServices.makeAPICall(apiServices.postMethod,apiServices.URL + apiServices.telacoach_certificationAPI,param,() => {});
      if(res.data.flag == 1) {
        this.certification_data = res.data.data.telacoach_certification_text
      }
      else {
