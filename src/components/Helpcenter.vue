        ...
        <div class="container pb-5">
            <div class="container box-shadow border-rounde mt-5 p-0 bg-white" style="width: 750px">
                <div class="">
                    ...
                    <div class="text-center h5 text-center font-weight-bold border-bottom pr-100 mb-0 pb-4">Help Center</div>
                    <div id="accordion" v-if="is_flag == '1'">
                        <div class="card" v-for="(item, index) in help_center" :key="index">
...
export default {
    ...
    methods: {
        async getHelpCenterData() {
            var param = "lang=" + "0";
            let res = await apiServices.makeAPICall(apiServices.postMethod,apiServices.URL + apiServices.help_center_listAPI,param,() => {});
            if(res.data.flag == 1) {
                this.is_flag = res.data.flag;
                Vue.prototype.$help_center_list = res.data.data;
                this.help_center = Vue.prototype.$help_center_list;
            }
            else {
                this.is_flag = res.data.flag;
